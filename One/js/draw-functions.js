...
export function drawWaveText(context) {
    const scaler = () => (waveTextY - 50) / (canvasDimesions.height / 2) + 0.2;

    if (intermissionInProgress()) {
        waveTextY = canvasDimesions.height / 2;
    } else if (waveTextY > 50) {
        waveTextY -= delta * 500 * scaler();
        waveTextY = Math.max(50, waveTextY);
    }

    context.fillStyle = "#888";
    context.fillStyle = `rgba(136,136,136,${scaler()})`;
    context.font = `${waveTextSize * scaler()}px ariel`;
    context.textAlign = "center";
    context.fillText(`Wave ${wave}`, canvasDimesions.width / 2, waveTextY);
}
